...
function saludo(idioma) {
  ...
  return 'Hola!'; // cualquier otro caso, incluso undefined
}
...
function estaEnRango(numero) {
  ...
  // De lo contrario, devuelve "false"
  // Tu código:
  return numero < 50 && numero > 20; // && significa Y (ambas deben cumplirse)
}
...
function fizzBuzz(numero) {
  ...
  // IMPORTANTE: primero chequeamos divisible por AMBOS
...
function esPrimo(numero) {
  ...
  for (let i = 2; i < numero; i++) {
    if (numero % i === 0) return false; // encontró un divisor, no es primo
  }
  return true; // ningún número lo dividió, es primo
}
...
function construirListaConConstMutando() {
  ...
  // Tu código:
  const lista = []; // const no impide hacer .push
  for (let i = 1; i <= 3; i++) {
    lista.push(i);
  }
  return lista;
...
function acumularPorPasos(inicial, pasos) {
  ...
  for (let i = 0; i < pasos.length; i++) {
    total += pasos[i]; // += suma al acumulador
  }
  return total;
}
...
function buscarIndiceCadena(texto, subcadena) {
  ...
  // Tu código:
  return texto.indexOf(subcadena); // posición donde empieza, o -1 si no existe
}

function extraerPorSlice(texto, inicio, fin) {
  // Usá slice exclusivo final `fin` igual que JS (no incluye el índice fin).
  // Tu código:
  return texto.slice(inicio, fin); // extrae desde inicio hasta fin (sin incluir fin)
}

function limpiarEspaciosExtremos(texto) {
  // Usá trim.
  // Tu código:
  return texto.trim(); // elimina espacios del inicio y final
}
...
function mayusculasTrasTrim(texto) {
  ...
  // Tu código:
  return texto.trim().toUpperCase(); // primero trim, luego mayúsculas
}
...
function primerMultiploEnIntervaloDoWhile(inicio, divisor, limiteSuperior) {
  ...
  let multiplicador = Math.ceil(inicio / divisor);
  let candidato;
  do {
    candidato = divisor * multiplicador;
    if (candidato > limiteSuperior) return null;
    multiplicador++;
